# Скрипт для мониторинга загрузки модели и статуса бота
...
while ($true) {
    ...
    if ($pythonProcess) {
        ...
        Write-Host "   Время запуска: $($pythonProcess.StartTime)" -ForegroundColor White
    } else {
    ...
    if (Test-Path "logs\bot.log") {
        ...
        $lastLogs = Get-Content "logs\bot.log" -Tail 5
