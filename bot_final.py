...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/bot_final.log', encoding='utf-8')
    ]
)
...
async def process_rag_query(question: str) -> str:
    ...
    try:
        ...
        # Вызываем QA chain в отдельном потоке
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, qa_chain, {"question": question})
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_state == "main":
        ...
        response = await process_rag_query(query_text)
        ...
        # Вызываем DeepSeek API в отдельном потоке
        loop = asyncio.get_event_loop()
        api_response = await loop.run_in_executor(None, call_deepseek_api, query_text)
...
async def initialize_rag_system():
    ...
    try:
        ...
        # Инициализируем векторное хранилище и получаем retriever
...
async def main():
    ...
    if not rag_initialized:
        logger.warning("RAG система не инициализирована. Бот будет работать только с DeepSeek API.")
    ...
    # Запуск бота
    logger.info("Starting final bot with RAG and DeepSeek API integration...")
