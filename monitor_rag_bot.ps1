# Скрипт мониторинга RAG бота
# Автор: AI Assistant
# Дата: 2025-08-19

Write-Host "🔍 Мониторинг RAG бота..." -ForegroundColor Cyan

# Проверяем, запущен ли бот
$botProcess = Get-Process | Where-Object {$_.ProcessName -like "*python*" -and $_.CommandLine -like "*bot_rag_only.py*"}
if ($botProcess) {
    Write-Host "✅ RAG бот запущен (PID: $($botProcess.Id))" -ForegroundColor Green
} else {
    Write-Host "❌ RAG бот НЕ запущен!" -ForegroundColor Red
    Write-Host "Запустите бот командой: .\start_rag_bot.ps1" -ForegroundColor Yellow
    exit 1
}

# Проверяем файлы логов
$logFile = "logs/bot_rag_only.log"
...
while ($true) {
    Clear-Host
    Write-Host "🔍 Мониторинг RAG бота - $(Get-Date -Format 'HH:mm:ss')" -ForegroundColor Cyan
    Write-Host "=" * 50 -ForegroundColor Gray
    
    # Проверяем процесс бота
    $botProcess = Get-Process | Where-Object {$_.ProcessName -like "*python*" -and $_.CommandLine -like "*bot_rag_only.py*"}
    if ($botProcess) {
        Write-Host "✅ RAG бот активен (PID: $($botProcess.Id))" -ForegroundColor Green
        ...
        Write-Host "❌ RAG бот остановлен!" -ForegroundColor Red
        break
    }
